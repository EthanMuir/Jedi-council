...
estimates, revision history, price-target dispersion, guidance vs consensus,
and historical surprise pattern. Revision *momentum* and *dispersion* matter
more than the level."""
...
council. You see ONLY analyst estimates: consensus EPS/revenue, revision
history, price-target dispersion, guidance-vs-consensus, and the historical
earnings-surprise pattern. No price chart, no news, no fundamentals beyond
what's implied by estimates.

The level of the estimate matters far less than its MOMENTUM (is the
consensus being revised up or down, and how fast) and its DISPERSION (wide
disagreement among analysts means low-confidence consensus, not a clean
signal). A consistent multi-quarter beat pattern is itself information;
treat one-off surprises with more caution than a repeated pattern.
"""
...
class EstimateScribeSeat:
    ...
    horizons = competent_horizons("estimate_scribe")
    ...
    async def deliberate(
        ...
        user_prompt = (
            f"Analyst estimates, horizon {ctx.horizon}. Data as of {e.as_of.isoformat()}.\n\n"
            ...
            f"Price target: mean {e.price_target_mean}, high {e.price_target_high}, "
            f"low {e.price_target_low}, dispersion {e.price_target_dispersion}\n"
            f"Guidance vs consensus: {e.guidance_vs_consensus}\n"
            f"Historical surprise pattern:\n{surprises}\n\n"
            "Weigh revision momentum and dispersion over the raw level, then give your verdict."
        )
