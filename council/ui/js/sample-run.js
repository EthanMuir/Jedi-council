// Made by scripts/make-sample-run.py -- a finished sample NVDA run for the
// welcome slides (welcome.js). Don't edit by hand.
const SAMPLE_RUN = {
 "ticker": "NVDA",
 "headline": "More likely to go up than down over all three periods, most of all over the next year. Next week is close to a toss-up.",
 "terms": {
  "short": {
   "window": "the next week",
   "p_bullish": 0.541,
   "summary": "Slightly more likely up than down. The price trend and what traders are betting on point up a little, but a single week is hard to call, so treat it as close to a toss-up.",
   "ticks": [
    {
     "seat_id": "technician",
     "p_bullish": 0.617,
     "weight": 1.0
    },
    {
     "seat_id": "fundamentalist",
     "p_bullish": 0.512,
     "weight": 0.2
    },
    {
     "seat_id": "catalyst_seer",
     "p_bullish": 0.552,
     "weight": 1.0
    },
    {
     "seat_id": "insider_reader",
     "p_bullish": 0.5,
     "weight": 0.4
    },
    {
     "seat_id": "senate_watcher",
     "p_bullish": 0.5,
     "weight": 0.21
    },
    {
     "seat_id": "flow_cartographer",
     "p_bullish": 0.507,
     "weight": 0.28
    },
    {
     "seat_id": "oracle_options",
     "p_bullish": 0.538,
     "weight": 0.9
    },
    {
     "seat_id": "macro_sage",
     "p_bullish": 0.5,
     "weight": 0.3
    },
    {
     "seat_id": "cross_market",
     "p_bullish": 0.541,
     "weight": 0.8
    },
    {
     "seat_id": "estimate_scribe",
     "p_bullish": 0.531,
     "weight": 0.5
    },
    {
     "seat_id": "analyst_ratings",
     "p_bullish": 0.526,
     "weight": 0.6
    },
    {
     "seat_id": "structure_archivist",
     "p_bullish": 0.5,
     "weight": 0.5
    }
   ]
  },
  "medium": {
   "window": "the next 3 months",
   "p_bullish": 0.537,
   "summary": "Leaning up. Analysts keep raising their expectations for the company's profits and their price targets, which often carries on for a few months.",
   "ticks": [
    {
     "seat_id": "technician",
     "p_bullish": 0.563,
     "weight": 0.42
    },
    {
     "seat_id": "fundamentalist",
     "p_bullish": 0.561,
     "weight": 0.6
    },
    {
     "seat_id": "catalyst_seer",
     "p_bullish": 0.534,
     "weight": 0.7
    },
    {
     "seat_id": "insider_reader",
     "p_bullish": 0.489,
     "weight": 0.8
    },
    {
     "seat_id": "senate_watcher",
     "p_bullish": 0.512,
     "weight": 0.49
    },
    {
     "seat_id": "flow_cartographer",
     "p_bullish": 0.536,
     "weight": 0.56
    },
    {
     "seat_id": "oracle_options",
     "p_bullish": 0.522,
     "weight": 0.6
    },
    {
     "seat_id": "macro_sage",
     "p_bullish": 0.519,
     "weight": 0.8
    },
    {
     "seat_id": "cross_market",
     "p_bullish": 0.524,
     "weight": 0.6
    },
    {
     "seat_id": "estimate_scribe",
     "p_bullish": 0.584,
     "weight": 0.9
    },
    {
     "seat_id": "analyst_ratings",
     "p_bullish": 0.577,
     "weight": 0.8
    },
    {
     "seat_id": "structure_archivist",
     "p_bullish": 0.514,
     "weight": 0.7
    }
   ]
  },
  "long": {
   "window": "the next year and beyond",
   "p_bullish": 0.543,
   "summary": "Leaning up. The business is growing and making more money, and analysts expect more. The stock is already priced high compared with similar companies, which is the main reason for caution.",
   "ticks": [
    {
     "seat_id": "technician",
     "p_bullish": 0.521,
     "weight": 0.3
    },
    {
     "seat_id": "fundamentalist",
     "p_bullish": 0.603,
     "weight": 1.0
    },
    {
     "seat_id": "catalyst_seer",
     "p_bullish": 0.5,
     "weight": 0.4
    },
    {
     "seat_id": "insider_reader",
     "p_bullish": 0.507,
     "weight": 0.9
    },
    {
     "seat_id": "senate_watcher",
     "p_bullish": 0.521,
     "weight": 0.56
    },
    {
     "seat_id": "flow_cartographer",
     "p_bullish": 0.553,
     "weight": 0.63
    },
    {
     "seat_id": "oracle_options",
     "p_bullish": 0.5,
     "weight": 0.3
    },
    {
     "seat_id": "macro_sage",
     "p_bullish": 0.536,
     "weight": 1.0
    },
    {
     "seat_id": "cross_market",
     "p_bullish": 0.508,
     "weight": 0.4
    },
    {
     "seat_id": "estimate_scribe",
     "p_bullish": 0.571,
     "weight": 0.9
    },
    {
     "seat_id": "analyst_ratings",
     "p_bullish": 0.612,
     "weight": 0.9
    },
    {
     "seat_id": "structure_archivist",
     "p_bullish": 0.483,
     "weight": 0.8
    }
   ]
  }
 },
 "seats": [
  {
   "seat_id": "technician",
   "p_bullish": 0.563,
   "rationale": "The 20/50-day uptrend is intact, but momentum signals usually fade over a quarter."
  },
  {
   "seat_id": "analyst_ratings",
   "p_bullish": 0.577,
   "rationale": "Most analysts rate it a buy, and the average target is well above today's price."
  },
  {
   "seat_id": "insider_reader",
   "p_bullish": 0.489,
   "rationale": "Steady planned selling with no open-market buys is a faint negative over months."
  }
 ]
};
